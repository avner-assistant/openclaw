  ...
  it.each(["missing", "nonzero", "timeout"] as const)(
    ...
    async (failure) => {
      for (const explicitHide of [undefined, "start", "update", "end"] as const) {
        ...
        const session = createProcessSessionFixture({ id: `poll-${failure}`, backgrounded: true });
        if (failure !== "missing") {
          addSession(session);
          markExited(
            session,
...
describe("sessions_yield channel progress privacy", () => {
  ...
  it.each([false, true])("preserves yield failures with explicit hide=%s", async (explicitHide) => {
    const { ctx } = createTestContext();
    const events: CapturedAgentEvent[] = [];
    const onYield = vi.fn();
    ctx.params.onAgentEvent = (event) => {
